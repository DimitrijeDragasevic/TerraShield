...
export async function createSimpleProposal(page) {
    ...
    await page.getByRole('button', { name: 'Update DAO information Update' }).click();
    await page.getByPlaceholder('Enter proposal title').fill('Test simple proposal');
    await page.getByPlaceholder('Enter proposal description').fill('Test simple proposal');
    await page.getByPlaceholder('Describe your DAO in 560').fill('update description');
    await expect(await page.getByRole("button", { name: "Next" })).toBeVisible();
    await page.getByRole("button", { name: "Next" }).click();
//    await page.getByText('General', { exact: true }).click();
//    await page.getByText('Text proposal').click();
//    await expect(page.getByText('Create general-purpose text')).toBeVisible();
