...
test('Swap 1 LUNA to AXLUSDT successfully', async ({page, homePage}) => {
    ...
    await expect(page.locator('article').filter({ hasText: 'SwapPowered by TFMSelect a chain to perform swaps onTerraLeave coins to pay fees' }).getByRole('article')).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Select a chain to perform swaps on' })).toBeVisible();
    ...
    const axlUSDTSpan = await page.locator('span.Read_component__2DP8V > .Read_small__37DWT')
                               .nth(2);
    ...
    expect(conversionRate).toBeCloseTo(expectedConversionRate, 1); // assuming ±1 variance
    ...
    // Min Received
    const minReceivedText = await page.textContent('YOUR_MIN_RECEIVED_SELECTOR');
    const actualMinReceived = parseFloat(minReceivedText.replace(' axlUSDT', '').trim());
    expect(actualMinReceived).toBeCloseTo(0.438749, 1);

    // Price Impact
    const priceImpactText = await page.textContent('YOUR_PRICE_IMPACT_SELECTOR');
    const actualPriceImpact = parseFloat(priceImpactText.replace('%', '').trim());
    expect(actualPriceImpact).toBeCloseTo(-2.44, 1);

    // Fee
    const feeText = await page.textContent('YOUR_FEE_SELECTOR');
    const actualFee = parseFloat(feeText.replace(' LUNA', '').trim());
    expect(actualFee).toBeCloseTo(0.003, 1);

    // Current Balance
    const currentBalanceText = await page.textContent('YOUR_CURRENT_BALANCE_SELECTOR');
